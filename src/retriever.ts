...
export interface RetrievalConfig {
  ...
  /** Enable intent analysis before retrieval (default: false) */
  intentAnalysisEnabled?: boolean;
  /** Minimax API key for intent analysis */
  minimaxApiKey?: string;
  /** Minimax model for intent analysis (default: MiniMax-Text-01) */
  minimaxModel?: string;
  /** Minimax base URL (default: https://api.minimax.chat/v1) */
  minimaxBaseURL?: string;
  /** Enable hierarchical retrieval (default: false) */
  hierarchicalRetrievalEnabled?: boolean;
}

// ============================================================================
// Intent Analysis Types (OpenViking Feature #1)
// ============================================================================

/** Query type for typed intent queries */
export type IntentQueryType = "skill" | "resource" | "memory" | "preference";

/** A typed query generated from intent analysis */
export interface IntentQuery {
  type: IntentQueryType;
  query: string;
  score: number; // confidence of this intent interpretation
}

/** Result of intent analysis */
export interface IntentAnalysis {
  primaryIntent: IntentQuery;
  alternativeIntents: IntentQuery[];
  analyzedQuery: string;
}

// ============================================================================
// Hierarchical Retrieval Types (OpenViking Feature #3)
// ============================================================================

/** A memory with propagated hierarchical score */
export interface HierarchicalSearchResult extends MemorySearchResult {
  pathDepth: number;
...
// ============================================================================
// Intent Analysis Helper Functions (OpenViking Feature #1)
// ============================================================================

/** Build the prompt for intent analysis */
function buildIntentAnalysisPrompt(query: string): string {
  return `Analyze the following user query and determine the search intent.

Query: "${query}"

Task: Determine what type of information the user is looking for. Generate 0-3 typed queries.

Types:
- "skill": User wants to find instructions, how-to guides, or procedural knowledge
- "resource": User wants to find references, documentation, or external resources
- "memory": User wants to recall past conversations, facts, or personal information
- "preference": User wants to find their own preferences or settings
...
Return JSON with:
{
  ...
  "analyzedQuery": "the original query or refined version"
}

Rules:
- If the query clearly matches one type, generate exactly 1 primaryIntent
- If ambiguous, generate up to 3 alternativeIntents with different type interpretations
- If no clear intent, return empty arrays and set analyzedQuery to the original
- Scores should reflect confidence; only include intents with score >= 0.3
- Maximum 3 total intents (1 primary + 2 alternatives)
`;
}
...
/** Parse intent analysis result from JSON */
function parseIntentAnalysisResult(
  data: any,
  ...
  if (!primaryIntent || !primaryIntent.type || !primaryIntent.query) {
    // No clear primary intent
    return {
      primaryIntent: {
        type: "memory",
        query: originalQuery,
      ...
      alternativeIntents: [],
      analyzedQuery: originalQuery,
    };
  }

  const validTypes = new Set(["skill", "resource", "memory", "preference"]);
  if (!validTypes.has(primaryIntent.type)) {
    primaryIntent.type = "memory";
  }

  const alternativeIntents: IntentQuery[] = [];

  if (Array.isArray(data.alternativeIntents)) {
    for (const alt of data.alternativeIntents) {
      if (
        alt &&
        alt.type &&
        validTypes.has(alt.type) &&
        alt.query &&
        ...
        alternativeIntents.push({
          type: alt.type,
          query: alt.query,
          score: Math.min(1, Math.max(0, alt.score)),
        });
      }
  ...
  return {
    primaryIntent: {
      ...
      query: primaryIntent.query,
      score: Math.min(1, Math.max(0, primaryIntent.score ?? 0.5)),
    },
    alternativeIntents,
    analyzedQuery: data.analyzedQuery || originalQuery,
  };
}

// ============================================================================
// Hierarchical Retrieval Helper Functions (OpenViking Feature #3)
// ============================================================================

/** Get the depth of a VikingFS path (number of segments) */
function getPathDepth(path: string): number {
  if (!path) return 0;
  // Path format: user/memories/category/subcategory/
  // Each segment between / counts as one depth level
  const segments = path.split("/").filter((s) => s.length > 0);
  return Math.max(0, segments.length - 1); // Exclude "user" root
}

/** Get the parent path of a given VikingFS path */
function getParentPath(path: string): string {
  if (!path) return "user/memories/";
  const segments = path.split("/").filter((s) => s.length > 0);
...
export class MemoryRetriever {
  ...
  // ============================================================================
  // Intent Analysis (OpenViking Feature #1)
  // ============================================================================

  /**
   * Analyze user query to understand intent before retrieval.
   * Generates 0-3 typed queries (skill/resource/memory/preference).
   * Uses minimax model for LLM-based intent classification.
   */
  async analyzeIntent(query: string): Promise<IntentAnalysis | null> {
    if (!this.config.intentAnalysisEnabled || !this.config.minimaxApiKey) {
      return null;
    }
    ...
    try {
      const response = await fetch(
        `${this.config.minimaxBaseURL}/text/chatcompletion_v2`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        console.warn(
          `memory-lancedb-pro: intent analysis failed: ${response.status} ${await response.text().catch(() => "")}`,
        );
        return null;
      }
      ...
      const intentData = JSON.parse(parsed);
      return parseIntentAnalysisResult(intentData, query);
    } catch (err) {
      console.warn(
        `memory-lancedb-pro: intent analysis error: ${err instanceof Error ? err.message : String(err)}`,
      );
      return null;
    }
  }
  ...
  // ============================================================================
  // Hierarchical Retrieval (OpenViking Feature #3)
  // ============================================================================

  /**
   * Hierarchical retrieval that traverses the VikingFS directory tree.
   * - First locates starting directory via vector search
   * - Then recursively retrieves subdirectories
   * - Score propagation: childScore = 50% embedding + 50% parentScore * 0.5
   * - Convergence: stops when consecutive layers differ by < 5%
   */
  async hierarchicalRetrieve(
    query: string,
    limit = 10,
    scopeFilter?: string[],
  ): Promise<HierarchicalSearchResult[]> {
    if (!this.config.hierarchicalRetrievalEnabled) {
      // Fallback to regular retrieval
      const results = await this.retrieve({ query, limit, scopeFilter });
      return results.map((r) => ({
        ...r,
        pathDepth: 0,
        propagatedScore: r.score,
      }));
    }

    // Step 1: Find starting directory via vector search
    const queryVector = await this.embedder.embedQuery(query);
    const startResults = await this.store.vectorSearch(
      queryVector,
      5,
      0.3,
      scopeFilter,
    ...
    // Find the best entry to start from (highest score with a path)
    const startEntry = startResults.find((r) => r.entry.path) ?? startResults[0];
    const startPath = startEntry.entry.path ?? "user/memories/";
    const startScore = startEntry.score;

    // Step 2: Get all memories under the starting path prefix
    const allResults = await this.store.hierarchicalSearch(startPath, 100, scopeFilter);

    if (allResults.length === 0) {
      return [];
    }

    // Step 3: Build directory tree from paths and compute depth
    const pathDepthMap = new Map<string, number>();
    for (const result of allResults) {
      const path = result.entry.path ?? startPath;
      const depth = getPathDepth(path);
      pathDepthMap.set(result.entry.id, depth);
    }

    // Step 4: Propagate scores hierarchically
    // Score propagation: childScore = 50% embedding + 50% parentScore * 0.5 decay
    const DECAY_FACTOR = 0.5;
    const EMBEDDING_WEIGHT = 0.5;
    const PARENT_WEIGHT = 0.5;

    const sortedByDepth = [...allResults].sort(
      (a, b) => (pathDepthMap.get(a.entry.id) ?? 0) - (pathDepthMap.get(b.entry.id) ?? 0),
    );

    const scoredMap = new Map<string, number>();
    let prevAvgScore = startScore;
    let converged = false;

    // First, set base scores from vector similarity
    for (const result of allResults) {
      const baseScore = result.score;
      const depth = pathDepthMap.get(result.entry.id) ?? 0;
      // Deeper entries start with lower base scores
      const depthFactor = Math.pow(DECAY_FACTOR, depth);
      scoredMap.set(
        result.entry.id,
        EMBEDDING_WEIGHT * baseScore + PARENT_WEIGHT * startScore * depthFactor,
      );
    }

    // Iterative propagation until convergence
    for (let iteration = 0; iteration < 10 && !converged; iteration++) {
      const currentAvg = [...scoredMap.values()].reduce((a, b) => a + b, 0) / scoredMap.size;
      converged = Math.abs(currentAvg - prevAvgScore) < 0.05;
      prevAvgScore = currentAvg;

      if (converged) break;

      // Update scores based on current averages (simplified propagation)
      for (const result of sortedByDepth) {
        const depth = pathDepthMap.get(result.entry.id) ?? 0;
        const embeddingScore = result.score;
        const depthFactor = Math.pow(DECAY_FACTOR, Math.max(0, depth - 1));

        // Find parent entry (one level up in path)
        const parentPath = getParentPath(result.entry.path ?? startPath);
        const parentResult = allResults.find(
          (r) => r.entry.path && r.entry.path === parentPath,
        );
        const parentScore = parentResult
          ? scoredMap.get(parentResult.entry.id) ?? startScore
          : startScore;

        const newScore =
          EMBEDDING_WEIGHT * embeddingScore +
          PARENT_WEIGHT * parentScore * depthFactor;

        scoredMap.set(result.entry.id, Math.min(1, Math.max(0, newScore)));
      }
    }

    // Step 5: Build final results with propagated scores
    const finalResults: HierarchicalSearchResult[] = allResults.map((result) => {
      const depth = pathDepthMap.get(result.entry.id) ?? 0;
      const propagatedScore = scoredMap.get(result.entry.id) ?? result.score;
      const parentPath = getParentPath(result.entry.path ?? startPath);
      const parentResult = allResults.find(
        (r) => r.entry.path && r.entry.path === parentPath,
      );
      ...
      return {
        ...
        pathDepth: depth,
        propagatedScore,
        parentScore: parentResult ? scoredMap.get(parentResult.entry.id) : undefined,
      };
    });

    // Sort by propagated score descending
    finalResults.sort((a, b) => b.propagatedScore - a.propagatedScore);
