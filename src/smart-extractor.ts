...
export class SmartExtractor {
  ...
  private async handleMerge(
    ...
    await this.store.update(
      matchId,
      {
        ...
        vector: newVector,
        l0_summary: merged.abstract,
        l1_summary: merged.overview,
        l2_content: merged.content,
        metadata,
      },
      scopeFilter,
  ...
  private async handleSupersede(
    ...
    const created = await this.store.store({
      ...
      l0_summary: candidate.abstract,
      l1_summary: candidate.overview,
  ...
  private async storeCandidate(
    ...
    // Store with L0/L1/L2 as explicit schema fields (OpenViking Feature #4)
    await this.store.store({
      ...
      scope: targetScope,
      importance: this.getDefaultImportance(candidate.category),
      l0_summary: candidate.abstract, // L0: 50-100 tokens 一句概括
      l1_summary: candidate.overview, // L1: 200-500 tokens 结构化摘要
      l2_content: candidate.content, // L2: 完整内容
      metadata,
    });
