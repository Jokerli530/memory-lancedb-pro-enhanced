...
/**
 * VikingFS-style path categories for hierarchical storage.
 * Format: user/memories/{category}/{subcategory}/
 */
...
/**
 * Generate a default VikingFS-style path for a memory entry based on its category.
 */
export function generateDefaultPath(category: MemoryEntry["category"], subcategory = "general"): string {
  // Map store categories to VikingFS path categories
  const categoryMap: Record<MemoryEntry["category"], PathCategory> = {
  ...
  const pathCategory = categoryMap[category] ?? "patterns";
  return `user/memories/${pathCategory}/${subcategory}/`;
}
...
export interface MemoryEntry {
  ...
  importance: number;
  timestamp: number;
  /** VikingFS-style hierarchical path, e.g. "user/memories/preferences/editor/" */
  path?: string;
  /** L0 summary: 50-100 tokens, one-line概括 */
  l0_summary?: string;
  /** L1 summary: 200-500 tokens, structured overview */
  l1_summary?: string;
  /** L2 full content (alias for text, stored for explicit schema clarity) */
  l2_content?: string;
...
export class MemoryStore {
  ...
  async store(
    ...
    const path = entry.path ?? generateDefaultPath(entry.category);
    // Auto-populate l2_content from text if not provided
    const l2_content = entry.l2_content ?? entry.text;
  ...
  /**
   * Hierarchical search: find all memories under a given path prefix.
   * Used by hierarchicalRetrieve() to traverse the VikingFS directory tree.
   */
  async hierarchicalSearch(
    ...
    return results.map((row): MemorySearchResult => {
      ...
      return { entry, score: 1.0 }; // Initial score, will be adjusted by hierarchical propagation
    });
  }
  ...
  async update(
    ...
    return this.runSerializedUpdate(async () => {
      ...
      const updated: MemoryEntry = {
        ...
        l0_summary: updates.l0_summary ?? original.l0_summary,
        l1_summary: updates.l1_summary ?? original.l1_summary,
        l2_content: updates.l2_content ?? original.l2_content,
